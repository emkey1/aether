...
def main():
    if len(sys.argv) != 4:
        sys.stderr.write("usage: gen-aokfs.py <manifest> <source-dir> <output.inc>\n")
        return 2
    ...
    for idx, (name, mode) in enumerate(entries):
        ...
        with open(src, "rb") as f:
            data = f.read()
        var = "aokfs_gen_data_%d" % idx
        out.append(emit_data(data, var))
        out.append("")
        table.append('    { "/tests/%s", %s, %uu, 0%ou },'
                     % (name, var, len(data), mode))

    out.append("static const struct aokfs_gen_file aokfs_gen_files[] = {")
    out.extend(table)
    out.append("};")
    out.append("")
    out.append("#define AOKFS_GEN_FILE_COUNT "
               "(sizeof(aokfs_gen_files) / sizeof(aokfs_gen_files[0]))")
    out.append("")
