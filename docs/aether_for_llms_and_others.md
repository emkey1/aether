...
12. **BUILT-001.** The builtins listed here are the supported, recommended
    ...
    listed, it does not exist" is something you can check rather than take on
...
## Appendix: complete builtin inventory

<!-- BEGIN GENERATED BUILTIN INVENTORY -->

*Generated by `tools/gen_builtin_appendix.py`. Do not edit by hand.*

Every builtin this compiler exposes: **110** with a documented signature, **302** more by name only. If a helper appears in neither list, it does not exist — inline the logic instead of reaching for it.
...
Safe to call exactly as written.

**TOON (structured data)**

| Signature | Effect |
|---|---|
| `toon_at(node: ToonNode, index: Int) -> ToonNode` | pure |
| `toon_bool_value(node: ToonNode) -> Bool` | pure |
| `toon_close(doc: ToonDoc) -> Void` | pure |
| `toon_free(node: ToonNode) -> Void` | pure |
| `toon_get_bool(node: ToonNode, key: Text) -> Bool` | pure |
| `toon_get_bool_or(node: ToonNode, key: Text, fallback: Bool) -> Bool` | pure |
| `toon_get_int(node: ToonNode, key: Text) -> Int` | pure |
| `toon_get_int_or(node: ToonNode, key: Text, fallback: Int) -> Int` | pure |
| `toon_get_real(node: ToonNode, key: Text) -> Real` | pure |
| `toon_get_real_or(node: ToonNode, key: Text, fallback: Real) -> Real` | pure |
| `toon_get_text(node: ToonNode, key: Text) -> Text` | pure |
| `toon_get_text_or(node: ToonNode, key: Text, fallback: Text) -> Text` | pure |
| `toon_has_at(node: ToonNode, index: Int) -> Bool` | pure |
| `toon_has_key(node: ToonNode, key: Text) -> Bool` | pure |
| `toon_int_value(node: ToonNode) -> Int` | pure |
| `toon_is_arr(node: ToonNode) -> Bool` | pure |
| `toon_is_bool(node: ToonNode) -> Bool` | pure |
| `toon_is_int(node: ToonNode) -> Bool` | pure |
| `toon_is_null(node: ToonNode) -> Bool` | pure |
| `toon_is_obj(node: ToonNode) -> Bool` | pure |
| `toon_is_real(node: ToonNode) -> Bool` | pure |
| `toon_is_text(node: ToonNode) -> Bool` | pure |
| `toon_key(node: ToonNode, key: Text) -> ToonNode` | pure |
| `toon_key_or(node: ToonNode, key: Text, fallback: ToonNode) -> ToonNode` | pure |
| `toon_len(node: ToonNode) -> Int` | pure |
| `toon_null() -> ToonNode` | pure |
| `toon_null_value(node: ToonNode) -> Bool` | pure |
| `toon_parse(payload: Text) -> ToonDoc` | pure |
| `toon_parse_file(path: Text) -> ToonDoc` | `fx` |
| `toon_real_value(node: ToonNode) -> Real` | pure |
| `toon_root(doc: ToonDoc) -> ToonNode` | pure |
| `toon_text_value(node: ToonNode) -> Text` | pure |
| `toon_type(node: ToonNode) -> Text` | pure |

**AI**

| Signature | Effect |
|---|---|
| `ai_chat(model: Text, messages: Text, system: Text = "", apiKey: Text = "", endpoint: Text = "") -> Text` | `fx` |

**Capability probes**

| Signature | Effect |
|---|---|
| `builtin_info(name: Text) -> Text` | pure |
| `builtins_json(detail: Bool = false) -> Text` | pure |
| `has_ai() -> Bool` | pure |
| `has_builtin(category: Text, function: Text) -> Bool` | pure |
| `has_toon() -> Bool` | pure |
...
| Signature | Effect |
|---|---|
| `len(arrayOrText) -> Int` | pure |

**Conversion**

| Signature | Effect |
|---|---|
| `formatfloat(value: Real, precision: Int) -> Text` | pure |
| `realtostr(value: Real) -> Text` | pure |

**Filesystem**

| Signature | Effect |
|---|---|
| `fileexists(path: Text) -> Bool` | `fx` |
| `getcurrentdir() -> Text` | `fx` |
| `mkdir(path: Text) -> Int` | `fx` |
| `mstreamloadfromfile(s: MStream, filename: Text) -> Void` | `fx` |
| `mstreamsavetofile(s: MStream, filename: Text) -> Void` | `fx` |
| `rmdir(path: Text) -> Int` | `fx` |
...
|---|---|
| `print(args...) -> Void` | `fx` |
| `println(args...) -> Void` | `fx` |

**Network and HTTP**

| Signature | Effect |
|---|---|
| `httpawait(id: Int, out: MStream) -> Int` | `fx` |
| `httpcancel(id: Int) -> Void` | `fx` |
| `httpclearheaders(session: Int) -> Void` | `fx` |
| `httpclose(session: Int) -> Void` | `fx` |
| `httperrorcode(session: Int) -> Int` | `fx` |
| `httpgetasyncprogress(id: Int) -> Int` | `fx` |
| `httpgetasynctotal(id: Int) -> Int` | `fx` |
| `httpgetheader(session: Int, name: Text) -> Text` | `fx` |
| `httpgetlastheaders(session: Int) -> Text` | `fx` |
| `httpisdone(id: Int) -> Bool` | `fx` |
| `httplasterror(session: Int) -> Text` | `fx` |
| `httprequest(session: Int, method: Text, url: Text, body: Text, out: MStream) -> Int` | `fx` |
| `httprequestasync(session: Int, method: Text, url: Text, body: Text) -> Int` | `fx` |
| `httprequestasynctofile(session: Int, method: Text, url: Text, body: Text, out: Text) -> Int` | `fx` |
| `httprequesttofile(session: Int, method: Text, url: Text, body: Text, out: Text) -> Int` | `fx` |
| `httpsession() -> Int` | `fx` |
| `httpsetheader(session: Int, name: Text, value: Text) -> Void` | `fx` |
| `httpsetoption(session: Int, key: Text, value: Int) -> Void` | `fx` |
| `httptryawait(id: Int, out: MStream) -> Int` | `fx` |

**SQLite**

| Signature | Effect |
|---|---|
| `sqlitebinddouble(stmt: Int, index: Int, value: Real) -> Int` | `fx` |
| `sqlitebindint(stmt: Int, index: Int, value: Int) -> Int` | `fx` |
| `sqlitebindnull(stmt: Int, index: Int) -> Int` | `fx` |
| `sqlitebindtext(stmt: Int, index: Int, value: Text) -> Int` | `fx` |
| `sqlitechanges(db: Int) -> Int` | `fx` |
| `sqliteclearbindings(stmt: Int) -> Int` | `fx` |
| `sqliteclose(db: Int) -> Int` | `fx` |
| `sqlitecolumncount(stmt: Int) -> Int` | `fx` |
| `sqlitecolumndouble(stmt: Int, column: Int) -> Real` | `fx` |
| `sqlitecolumnint(stmt: Int, column: Int) -> Int` | `fx` |
| `sqlitecolumnname(stmt: Int, column: Int) -> Text` | `fx` |
| `sqlitecolumntext(stmt: Int, column: Int) -> Text` | `fx` |
| `sqlitecolumntype(stmt: Int, column: Int) -> Int` | `fx` |
| `sqliteerrmsg(db: Int) -> Text` | `fx` |
| `sqliteexec(db: Int, sql: Text) -> Int` | `fx` |
| `sqlitefinalize(stmt: Int) -> Int` | `fx` |
| `sqlitelastinsertrowid(db: Int) -> Int` | `fx` |
| `sqliteopen(path: Text) -> Int` | `fx` |
| `sqliteprepare(db: Int, sql: Text) -> Int` | `fx` |
| `sqlitereset(stmt: Int) -> Int` | `fx` |
| `sqlitestep(stmt: Int) -> Int` | `fx` |

**Strings**

| Signature | Effect |
|---|---|
| `copy(s: Text, start: Int, count: Int) -> Text` | pure |
| `pos(needle: Text, s: Text) -> Int` | pure |
| `stringofchar(ch: Text, n: Int) -> Text` | pure |
| `trim(s: Text) -> Text` | pure |

**System and process**

| Signature | Effect |
|---|---|
| `getenv(name: Text) -> Text` | `fx` |
| `getenvint(name: Text, fallback: Int) -> Int` | `fx` |
| `paramcount() -> Int` | `fx` |
| `paramstr(i: Int) -> Text` | `fx` |
| `random() -> Real in [0,1); random(n: Int) -> Int in [0,n)` | `fx` |
| `randomize() -> Void` | `fx` |
...
| Signature | Effect |
|---|---|
| `task_lookup(name: Text) -> Int` | `fx` |
| `task_queue(target: Text, name: Text, arg) -> Int` | `fx` |
| `task_result(handle: Int, consume: Bool = false) -> Int` | `fx` |
| `task_spawn(target: Text, name: Text, arg) -> Int` | `fx` |
| `task_stats() -> Array` | `fx` |
| `task_stats_json() -> Text` | pure |
| `task_status(handle: Int, consume: Bool = false) -> Int` | `fx` |
| `task_wait(handle: Int) -> Int` | `fx` |

**Text**

| Signature | Effect |
|---|---|
| `string_len(text: Text) -> Int` | pure |

**Clock**

| Signature | Effect |
|---|---|
| `sleep(ms: Int) -> Void` | `fx` |
...
**Core** — `abs`, `apiReceive` *fx*, `apiSend` *fx*, `append`, `arccos`, `arcsin`, `arctan`, `assign`, `assignfile` *fx*, `atan2`, `beep` *fx*, `biblinktext`, `biboldtext`, `biclrscr`, `bilowvideo`, `binormvideo`, `biunderlinetext`, `biwherex`, `biwherey`, `blinktext`, `blockread` *fx*, `blockwrite` *fx*, `boldtext`, `bool`, `byte`, `bytecodeversion`, `ceil`, `channelclose`, `channelcreate`, `channelisclosed`, `channelreceive`, `channelsend`, `channeltryreceive`, `channeltrysend`, `char`, `chr`, `clamp`, `cleardevice`, `close`, `closefile` *fx*, `closegraph`, `closegraph3d`, `clreol`, `clrscr`, `cos`, `cosh`, `cotan`, `createtargettexture`, `createtexture`, `cursoroff`, `cursoron`, `dec`, `deline`, `destroytexture`, `dispose`, `dnslookup` *fx*, `dosExec` *fx*, `dosFindfirst` *fx*, `dosFindnext` *fx*, `dosGetdate` *fx*, `dosGetenv` *fx*, `dosGetfattr` *fx*, `dosGettime` *fx*, `dosMkdir` *fx*, `dosRmdir` *fx*, `double`, `drawcircle`, `drawline`, `drawpolygon`, `drawrect`, `eof` *fx*, `erase` *fx*, `exec` *fx*, `exit` *fx*, `exp`, `extbuiltincategorycount`, `extbuiltincategoryname`, `extbuiltinfunctioncount`, `extbuiltinfunctionname`, `extbuiltingroupcount`, `extbuiltingroupfunctioncount`, `extbuiltingroupfunctionname`, `extbuiltingroupname`, `fclose` *fx*, `fflush` *fx*, `filesize` *fx*, `fillcircle`, `fillrect`, `findfirst` *fx*, `findnext` *fx*, `float`, `floor`, `flush` *fx*, `fopen` *fx*, `format`, `fprintf` *fx*, `freesound`, `getdate` *fx*, `getfattr` *fx*, `getmaxx`, `getmaxy`, `getmousestate`, `getpixelcolor`, `getscreensize`, `gettextsize`, `getticks`, `gettime` *fx*, `glbegin`, `glclear`, `glclearcolor`, `glcleardepth`, `glcolor3f`, `glcullface`, `gldepthfunc`, `gldepthmask`, `gldepthtest`, `glend`, `glfrustum`, `gllinewidth`, `glloadidentity`, `glmatrixmode`, `glperspective`, `glpopmatrix`, `glpushmatrix`, `glrotatef`, `glscalef`, `glsetswapinterval`, `glswapwindow`, `gltranslatef`, `glvertex3f`, `glviewport`, `gotoxy`, `graphloop`, `halt` *fx*, `hidecursor`, `high`, `highvideo`, `inc`, `initgraph`, `initgraph3d`, `initsoundsystem`, `inittextsystem`, `insline`, `int`, `inttostr`, `invertcolors`, `ioresult` *fx*, `iskeydown`, `issoundplaying`, `jsonget`, `keypressed` *fx*, `landscapesetlightingpreset`, `landscapesetpalettepreset`, `ln`, `loadimagetotexture`, `loadsound`, `log10`, `low`, `lowvideo`, `max`, `min`, `mstreamappendbyte`, `mstreambuffer`, `mstreamcreate`, `mstreamfree`, `mstreamfromstring`, `new`, `newobj`, `normalcolors`, `normvideo`, `odd`, `ord`, `outtextxy`, `playsound`, `pollkey`, `pollkeyany`, `popscreen`, `pow`, `power`, `printf` *fx*, `pushscreen`, `putpixel`, `quitrequested`, `quitsoundsystem`, `quittextsystem`, `read` *fx*, `readkey` *fx*, `readln` *fx*, `real`, `rename` *fx*, `rendercopy`, `rendercopyex`, `rendercopyrect`, `rendertexttotexture`, `reset`, `restorecursor`, `rewrite`, `round`, `savecursor`, `screencols`, `screenrows`, `setalphablend`, `setcolor`, `setlength`, `setrendertarget`, `setrgbcolor`, `showcursor`, `sin`, `sinh`, `sizeof`, `socketaccept`, `socketbind`, `socketbindaddr`, `socketclose`, `socketconnect`, `socketcreate`, `socketlasterror`, `socketlisten`, `socketpeeraddr`, `socketpoll`, `socketreceive`, `socketsend`, `socketsetblocking`, `sqr`, `sqrt`, `stopallsounds`, `str`, `succ`, `tan`, `tanh`, `taskawait` *fx*, `taskcancel` *fx*, `taskdone` *fx*, `taskspawn` *fx*, `textbackground`, `textbackgrounde`, `textcolor`, `textcolore`, `thread_cancel` *fx*, `thread_pause` *fx*, `thread_resume` *fx*, `thread_set_name` *fx*, `threadcancel`, `threadgetresult`, `threadgetstatus`, `threadlookup`, `threadpause`, `threadpoolsubmit`, `threadresume`, `threadsetname`, `threadspawnbuiltin` *fx*, `threadstats`, `threadstatsjson`, `tobool`, `tobyte`, `tochar`, `todouble`, `tofloat`, `toint`, `toupper`, `trunc`, `underlinetext`, `upcase`, `updatescreen`, `updatetexture`, `val`, `valreal`, `vmversion`, `waitforthread` *fx*, `waitkeyevent`, `wherex`, `wherey`, `window`

**math** — `chudnovsky`, `factorial`, `fibonacci`, `mandelbrotrow`

**Strings** — `atoi`, `itoa`, `parse_bool`, `parse_float`, `parse_int`, `split`

**System and process** — `getpid` *fx*, `realtimeclock` *fx*, `swap` *fx*

**yyjson** — `yyjsondocfree`, `yyjsonfreevalue`, `yyjsongetbool`, `yyjsongetindex`, `yyjsongetint`, `yyjsongetkey`, `yyjsongetlength`, `yyjsongetnumber`, `yyjsongetroot`, `yyjsongetstring`, `yyjsongettype`, `yyjsonhasindex`, `yyjsonhaskey`, `yyjsonisnull`, `yyjsonread`, `yyjsonreadfile` *fx*

<!-- END GENERATED BUILTIN INVENTORY -->
