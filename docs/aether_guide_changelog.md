...
|---|---|---|---|
| `2026-09-05-1` | 2026-09-05 | `4c3f01e` | feat: **foreach loops** (`loop x in xs` over arrays, `Text` and `ToonNode` arrays), **`step`** on ranges, the word operators `and`/`or`/`not`, structural array `==`; **reserved-word rules rewritten** — the shared lexer's foreign keywords (`join`, `match`, `class`, ...) and type-name words (`word`, `text`, `int`, ...) are ordinary identifiers, only Aether's own keywords and operator words are reserved, and the SYN-001 repair bullet notes the message now names the Aether form; **TYPE-002** replaces the SCOPE-001 spelling for an unknown type name and names `Char`/`Byte`/`Word`/`Str` and lowercase `int`/`string` (folds in the unshipped 2026-08-17 draft: `Float`/`String` as accepted aliases, the completed "codes the compiler actually emits" list, the range-first Loops lead, `inttostr` in the Int→Text column); **KEY-001 teaches dotted TOON paths** and NEST-001 states that a missing intermediate degrades; `setlength`, the numeric-cast spellings, `toon_key_or`/`toon_null`, `s[i] = "x"` and arrays of records inside records documented; appendix regenerated. Net +1,942 tokens: 26,950 → 28,892, no ceiling |
| `2026-08-11-9` | 2026-08-11 | `dd3ba75` | docs: **ORDER-001 retired entirely**, along with the silent-skip warning in **Concurrency: `par`**. The compiler was fixed (language `2026-08-11-3`, pscal-core `b06211f`): a `par` branch now resolves its target like any other call, so top-level declaration order is free with no exception left to state. The rule's other clause — locals still need `let` before use — is already carried by SCOPE-001 (rule 13 here), so nothing true was lost; rules renumbered 1–22. Note the previous row's verification matrix stands, only its `par` exception is void. Net −149 tokens: 27,099 → 26,950, no ceiling |
| `2026-08-11-8` | 2026-08-11 | `652fd4c`, `32b3e9f`, `519ce8b` | docs: **ORDER-001 narrowed to `par` branches**, the one case where declaration order still matters. The `par` section adds that the constraint is positional within the file, so an imported export called from a branch is fine (verified: a branch calling an import directly runs; a local wrapper around one is skipped only if the wrapper itself sits below). States *top-level* order rather than order in general, because locals remain ordered: reading a local above its `let` is a genuine `[SCOPE-001] identifier 'x' not in scope`. That also keeps the surviving "a genuinely undeclared / out-of-scope name → declare it earlier" repair bullet correct, since it is about locals. Verified against compiler `2026-08-11-2`, the old rule ("define types and helper functions before `main` uses them") was false in every position tested: a type used above its declaration in a parameter, return type, binding, array annotation or record field; a helper called by a top-level `fn`, an `fx` block or a method; mutual recursion in both directions; a `@pre` calling a helper defined later; and a `use` line placed *below* the function referencing its exports. All compile and run clean. A `par` branch is the exception and fails silently rather than loudly — a branch naming a function defined further down is **skipped with no error, no output, exit 0**, and the target must precede the function *containing* the `par`, not just the call site. Also **drops the SCOPE-001 repair bullet** "a type or helper used before it is defined → define it earlier": forward references never emit SCOPE-001, and the bullet contradicted the unknown-type bullet four lines below it, so a model repairing a SCOPE-001 was told both to move code and that moving code was pointless |
...
| version | date | commit | change |
|---|---|---|---|
| `2026-09-05-1` | 2026-09-05 | `4c3f01e` | feat: foreach loops and `step`, word operators `and`/`or`/`not`, structural array `==`; reserved-word rules rewritten (other languages' words are ordinary names); TYPE-002 replaces the SCOPE-001 spelling for an unknown type name and names `Char` and lowercase `int` (folds in the unshipped 2026-08-17 draft: `Float`/`String` aliases, range-first Loops lead); KEY-001 teaches dotted TOON paths, NEST-001 rewritten around the two total-path forms; `setlength`, cast spellings, `toon_key_or`/`toon_null`, `s[i] = "x"` documented. Net +136 tokens: 14,849 → 14,985 against the 15,000 ceiling, leaving 15 — paid for by one-line KEY-001/NEST-001 examples, the cast spellings folded into the `int(x)` sentence, the shorter reserved-word bullet, a one-sentence precedence note, one-liner array-printing and getter-on-a-`ToonDoc` examples, dropping the "a `type` may be declared after another `type`" bullet that ORDER-001's retirement made moot, dropping the duplicated single-`println` Effects block, and shortening the inference paragraph |
| `2026-08-11-4` | 2026-08-11 | `dd3ba75` | docs: **ORDER-001 retired entirely** (rule 10 here), along with the silent-skip line in **Concurrency: `par`**. The compiler was fixed (language `2026-08-11-3`, pscal-core `b06211f`): a `par` branch now resolves its target like any other call, so there is no ordering exception left to state. The rule's locals clause is already carried by SCOPE-001, now rule 13; rules renumbered 1–25. Net **−73 tokens: 14,922 → 14,849**, so headroom against the 15,000 ceiling goes from 78 to **151** — this is a repayment, not a spend, and the paired-cut constraint still binds the next addition. The `par` example is unchanged and still runs (`a=100 b=200`), verified by hand since the snippet gate is `--no-run` |
...
| version | date | commit | change |
|---|---|---|---|
| `2026-09-05-1` | 2026-09-05 | `4c3f01e` | feat: foreach loops and `step`, word operators `and`/`or`/`not`, structural array `==`; reserved-word rules rewritten (other languages' words are ordinary names); TYPE-002 names `Char` and lowercase `int` (folds in the unshipped 2026-08-17 draft: `Float`/`String` aliases, range-first Loops lead); KEY-001 teaches dotted TOON paths, NEST-001 rewritten; `setlength`, cast spellings, `toon_key_or`/`toon_null`, `s[i] = "x"` documented. Net +364 tokens: 11,534 → 11,898, still measured against a stated ~9K that remains un-re-baselined |
| `2026-08-11-7` | 2026-08-11 | `dd3ba75` | docs: **ORDER-001 retired entirely** (rule 10 here), along with the silent-skip line under the `par` example. The compiler was fixed (language `2026-08-11-3`, pscal-core `b06211f`): a `par` branch now resolves its target like any other call, so there is no ordering exception left to state. The rule's locals clause is already carried by SCOPE-001, now rule 14; rules renumbered 1–25. Net **−97 tokens: 11,631 → 11,534**, still measured against a stated ~9K that remains un-re-baselined |
| `2026-08-11-6` | 2026-08-11 | `652fd4c`, `32b3e9f` | docs: **ORDER-001 narrowed to `par` branches** (see the full-guide row for the verification matrix). Rule 10 restated from "define types, helpers, and modules before `main` uses them" to "top-level order is free" with the locals caveat, and the silent-skip warning added to **Concurrency: `par`**. Also **drops the SCOPE-001 repair bullet** "a type or helper used before it is defined → define it earlier", which contradicted the unknown-type bullet a few lines below it. Net **+18 tokens: 11,613 → 11,631**, part-paid by cutting the "capture-free way to parallelize user code" tail whose steer rule 24 (FUNC-001) already carries verbatim. Still measured against a stated ~9K that remains un-re-baselined |
