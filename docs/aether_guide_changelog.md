...
| version | date | commit | change |
|---|---|---|---|
| `2026-08-11-2` | 2026-08-11 | `652fd4c` | docs: **ORDER-001 narrowed to `par` branches** (see the full-guide row for the verification matrix). Rule 10 restated from "define types, helpers, and modules before `main` uses them" to "definition order is free, except in `par`", with the silent-skip warning added to **Concurrency: `par`**. Also **fixes a crashing example**: the `par` sample named its type `Tally` and its function `tally`, and a case-insensitive collision between a type name and a function taking that type as a parameter aborts with a VM slot-window error — the sample had never run. Renamed the function to `bump`; it now prints `a=100 b=200`. The full guide's copy used `type Box` and was unaffected. `verify_guide_snippets.py` could not have caught this: it compiles with `--no-run`. Net **−1 token: 14,922 → 14,921** against the 15,000 ceiling, honouring the paired-cut constraint — the cut is the "capture-free way to parallelize user code" tail, whose steer is already carried by rule 12 (FUNC-001) |
...
| version | date | commit | change |
|---|---|---|---|
| `2026-08-11-3` | 2026-08-11 | `652fd4c` | docs: **ORDER-001 narrowed to `par` branches** (see the full-guide row for the verification matrix). Rule 10 restated from "define types, helpers, and modules before `main` uses them" to "definition order is free, except in `par`", with the silent-skip warning added to **Concurrency: `par`**. Also **drops the SCOPE-001 repair bullet** "a type or helper used before it is defined → define it earlier", which contradicted the unknown-type bullet a few lines below it. Net **−12 tokens: 11,613 → 11,601**, paired-cut against the "capture-free way to parallelize user code" tail whose steer rule 24 (FUNC-001) already carries verbatim. Still measured against a stated ~9K that remains un-re-baselined |
