...
| version | date | commit | change |
|---|---|---|---|
| `2026-08-10-1` | 2026-08-10 | *(pending)* | docs: **NEST-001 rewritten around `toon_key_or(node, key, toon_null())`** — the total-path idiom replaces the guard-stack example as the primary shape, with the `toon_has_at` exception noted. `toon_key_or`/`toon_null` were absent from this tier entirely (not even an appendix row), which is the tier every benchmark run uses. Net +182 tokens; measured 14,833 against the 15,000 ceiling, so **headroom is down to 167** and this guide needs a trim pass before the next addition |
...
| version | date | commit | change |
|---|---|---|---|
| `2026-08-10-1` | 2026-08-10 | *(pending)* | docs: NEST-001 leads with the `toon_key_or(node, key, toon_null())` total-path idiom; the `if toon_has_key` guard stays as the one-line alternative, plus the `toon_has_at` null-node exception |
