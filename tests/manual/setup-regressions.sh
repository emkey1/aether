...
all_tests="signal_core signal_restart signal_realtime signal_altstack signal_stop_cont signal_forced_trap signal_poll signal_child_burst eventfd_interrupt futex_core process_lifecycle pthread_sync ptrace_group_stop ptrace_thread_follow epoll_mod_wake epoll_oneshot_rearm epoll_mod_spurious_wake epoll_data_layout epoll_eloop epoll_exclusive epoll_dup_add ptrace_exit_kill fcntl_lock fcntl_ofd fcntl_setown at_empty_path utimensat_fd copy_file_range name_to_handle_at sendfile_vhangup pidfd_open pidfd_clone pidfd_fdinfo_pid fsopen_move_mount keyctl_link mountinfo_epollet tmpfs_nlink unix_dgram_cred ambient_caps scm_rights_stress fs_conformance process_conformance time_conformance mem_conformance sock_conformance getpeername_smallbuf netlink_route netlink_audit mount_flags clone_error_cleanup posix_timer_fork concurrent_exec_tlb random_seed getrusage_group pty_line_discipline proc_pid_io taskstats_genl tmpfs_mmap tmpfs_statfs memfd_mmap mount_stdev mmap_truncate_sigbus signalfd_epoll_deadlock signalfd_thread_group wayland_scm_shm chroot_getcwd iovec_abi_marshal fakefs_type_race fifo_open_creat_deadlock proc_stat_monotonic sock_getfd_errno inotify_close_race inotify_mount_paths statx_mnt_id_timerfd opath_symlink_pidfd_wait pidfd_self_exit_deadlock prctl_capbset_drop oom_score_adj sysv_ipc accept_rcvtimeo procfd_reopen"
if [ "$is_x86_guest" -eq 1 ]; then
