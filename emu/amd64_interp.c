...
restart_prefix:
    ...
    case 0x0f: {
        ...
        if (op2 == 0x10 || op2 == 0x11 || op2 == 0x16 || op2 == 0x17 || op2 == 0x28 || op2 == 0x29 || op2 == 0x62 || op2 == 0x6c || (op2 == 0x6e && operand_size_prefix) || op2 == 0x6f || op2 == 0x70 || op2 == 0x7e || op2 == 0x7f || op2 == 0xc6 || op2 == 0xd6 || op2 == 0xeb || op2 == 0xef) {
            struct amd64_modrm modrm;
            union xmm_reg value;
        ...
        if (op2 == 0x6e) {
            ...
            if (modrm.reg >= 8)
                return INT_UNDEFINED;
            if (!amd64_read_rm(cpu, tlb, &modrm, fs_prefix, rex.w ? 64 : 32, &src))
                goto amd64_gpf_restore;
            cpu->mm[modrm.reg].qw = rex.w ? src : (uint32_t) src;
            break;
        }
        if (op2 == 0xb1) {
            struct amd64_modrm modrm;
